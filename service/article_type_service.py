...
class ArticleTypeService():
    @staticmethod
    def get_article_type(db_session, name):
        return db_session.query(ArticleType).filter(ArticleType.name == name).first()
    ...
    def add_article_type(db_session, article_type):
        try:
            type_do = ArticleTypeService.get_article_type(db_session, article_type["name"])
            if type_do:
                return False
    ...
    def update_article_type(db_session, article_type_id, article_type):
        try:
            other = db_session.query(ArticleType).filter(and_(ArticleType.id != article_type_id, ArticleType.name == article_type['name'])).first()
            if other:
                return False
